run "test_forge_code_basic" {
  ...
  assert {
    condition     = var.install_forge_code == true
    error_message = "Install forge_code should default to true"
  }
...
run "test_forge_code_with_api_key" {
  command = plan

  variables {
    agent_id       = "test-agent-456"
    workdir        = "/home/coder/workspace"
    forge_api_key = "test-api-key-123"
  }

  assert {
    condition     = coder_env.forge_api_key[0].value == "test-api-key-123"
    error_message = "forge API key value should match the input"
  }
}

run "test_forge_code_with_custom_options" {
  command = plan
  ...
  assert {
    ...
    error_message = "forge model variable should be set to 'opus'"
  }
  ...
  assert {
    condition     = var.forge_code_version == "1.0.0"
    error_message = "Forge Code version should be set to '1.0.0'"
  }
...
run "test_forge_code_with_mcp_and_tools" {
  command = plan
...
run "test_forge_code_with_scripts" {
  command = plan
...
run "test_forge_code_permission_mode_validation" {
  command = plan
...
run "test_forge_code_auto_permission_mode" {
  command = plan
...
run "test_forge_code_with_boundary" {
  command = plan
...
run "test_forge_code_system_prompt" {
  command = plan
  ...
  assert {
    condition     = trimspace(coder_env.forge_code_system_prompt.value) != ""
    error_message = "System prompt should not be empty"
  }

  assert {
    condition     = length(regexall("Custom addition", coder_env.forge_code_system_prompt.value)) > 0
    error_message = "System prompt should have system_prompt variable value"
  }
}

run "test_forge_report_tasks_default" {
  command = plan
  ...
  assert {
    condition     = startswith(trimspace(coder_env.forge_code_system_prompt.value), "<system>")
    error_message = "System prompt should start with <system>"
  }
  assert {
    condition     = endswith(trimspace(coder_env.forge_code_system_prompt.value), "</system>")
    error_message = "System prompt should end with </system>"
  }

  # Ensure Coder sections are injected when report_tasks=true (default)
  assert {
    condition     = length(regexall("-- Tool Selection --", coder_env.forge_code_system_prompt.value)) > 0
    error_message = "System prompt should have Tool Selection section"
  }

  assert {
    condition     = length(regexall("-- Task Reporting --", coder_env.forge_code_system_prompt.value)) > 0
    error_message = "System prompt should have Task Reporting section"
  }
}

run "test_forge_report_tasks_disabled" {
  command = plan
...
run "test_aibridge_enabled" {
  ...
  assert {
    condition     = coder_env.forge_api_key[0].name == "forge_API_KEY"
    error_message = "forge_API_KEY environment variable should be set"
  }

  assert {
    condition     = coder_env.forge_api_key[0].value == data.coder_workspace_owner.me.session_token
    error_message = "forge_API_KEY should use workspace owner's session token when aibridge is enabled"
  }
}
...
run "test_aibridge_validation_with_oauth_token" {
  ...
  variables {
    ...
    workdir                 = "/home/coder/test"
    enable_aibridge         = true
    forge_code_oauth_token = "test-oauth-token"
  }
...
run "test_aibridge_disabled_with_api_key" {
  ...
  variables {
    ...
    forge_api_key  = "test-api-key-xyz"
  }
  ...
  assert {
    condition     = coder_env.forge_api_key[0].value == "test-api-key-xyz"
    error_message = "forge_API_KEY should use the provided API key when aibridge is disabled"
  }
...
run "test_no_api_key_no_env" {
  ...
  assert {
    condition     = length(coder_env.forge_api_key) == 0
    error_message = "forge_API_KEY should not be created when no API key is provided and aibridge is disabled"
  }
}
...
run "test_api_key_count_with_aibridge_no_override" {
  ...
  assert {
    condition     = length(coder_env.forge_api_key) == 1
    error_message = "forge_API_KEY env should be created when aibridge is enabled, regardless of session_token value"
  }
}