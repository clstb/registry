...
function validate_forge_installation() {
  if command_exists forge; then
    printf "Forge Code is installed\n"
  else
    printf "Error: Forge Code is not installed. Please enable install_forge_code or install it manually\n"
    exit 1
  fi
}
...
get_project_dir() {
  ...
  echo "$HOME/.forge/projects/${workdir_normalized}"
}
...
is_valid_session() {
  ...
  # forge session files use JSONL (JSON Lines) format where each line is valid JSON
  ...
  # This ensures the file structure matches forge's session format
...
function start_agentapi() {
  ...
  printf "Running forge code with args: %s\n" "$(printf '%q ' "${ARGS[@]}")"
  ...
  if [ "$ARG_ENABLE_BOUNDARY" = "true" ]; then
    ...
    agentapi server --type forge --term-width 67 --term-height 1190 -- \
      "${BOUNDARY_CMD[@]}" "${BOUNDARY_ARGS[@]}" -- \
      forge "${ARGS[@]}"
  else
    agentapi server --type forge --term-width 67 --term-height 1190 -- forge "${ARGS[@]}"
  fi
}

validate_forge_installation
start_agentapi
